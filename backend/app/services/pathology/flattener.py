...
- v1: {"RBC": {...}, "Hb%": {...}, "unmatched_tests": [...]}
- v2: {"tests": [{standard_name, original_name, status, ...}, ...]}
...
from app.services.pathology.config import STANDARD_PARAMETERS, get_config_range
...
def flatten_standardized(standardized: Dict[str, Any]) -> List[PathologyField]:
    ...
    Supports v2 format (tests array) and falls back to v1 format.
    ...
    """
    # Detect format: v2 has "tests" array
    if "tests" in standardized:
        return _flatten_v2(standardized)
    return _flatten_v1(standardized)
...
def _flatten_v1(standardized: Dict[str, Any]) -> List[PathologyField]:
    """
    Flatten v1 format: {"RBC": {...}, "Hb%": {...}, "unmatched_tests": [...]}

    Legacy format with fixed parameter order.
    """
    fields: List[PathologyField] = []

    # ── Standard parameters ──────────────────────────────────────────────
    for param_name in STANDARD_PARAMETERS:
        data = standardized.get(param_name)
        if not isinstance(data, dict):
            continue  # Skip missing params (no placeholders)

        value = data.get("value")
        report_range = data.get("range") or STANDARD_PARAMETERS[param_name].get("range")

        # Get config range
        config_range = _get_config_range_str(param_name, None)
        ...
        fields.append(PathologyField(
            ...
            key=param_name,
            value=value,
            unit=data.get("unit") or STANDARD_PARAMETERS[param_name].get("unit"),
            reference_range=report_range,
            config_range=config_range,
            range_status=range_status,
            flag=data.get("flag"),
    ...
    # ── Remark ───────────────────────────────────────────────────────────
    remark = standardized.get("Remark")
    ...
    # ── Unmatched tests ──────────────────────────────────────────────────
    unmatched = standardized.get("unmatched_tests", [])
    if isinstance(unmatched, list):
        for test in unmatched:
            ...
            # Unmatched tests don't have config range
            range_status = compute_range_status(value, report_range, None)

            fields.append(PathologyField(
                id=str(uuid.uuid4()),
                key=test.get("name", "Unknown"),
                value=value,
                unit=test.get("unit"),
                reference_range=report_range,
                config_range=None,
                ...
                flag=test.get("flag"),
                method=test.get("method"),
                reference_name=test.get("name"),
                section_path=test.get("section_path", []),
                is_standard=False,
                source=FieldSource.LLM,
            ))
